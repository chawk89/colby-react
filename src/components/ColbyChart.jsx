...
import useAnnotationDragger, {
  hideColbyMenu,
  useMarkColbyChartOptions,
} from "../hooks/useAnnotationDragger";
import ChartMenu from "./common/ChartMenu";
import {
  findNearestDataPoint,
  getChartKey,
  getDatasetIndexWithoutXAxis,
} from "../utils/utils";
...
const ColbyChart = () => {
  ...
  const { state, chartRef, dispatch } = context;
  const { options, forms, data, chartType, getChart } = state;
  ...
  const handleClick = (event, data) => {
    ...
    const datasetKeys = getDatasetIndexWithoutXAxis(
    ...
    hideColbyMenu();
    dispatch({
      type: "CREATE_ANNOTATION_ITEM_BY_CONTEXTMENU",
      data: {
        ...data,
        x: xValue,
        y: yValue,
        nearestData: { ...nearestData, datasetKey },
      },
    });
  };
  const { indexAxis } = options;
  const [prev, setPrev] = useState(indexAxis);
